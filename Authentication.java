...
public class Authentication {
    ...
    public Admin signupAdmin(String userId, String username, String email, String plainPassword) throws IOException {
        if (emailExists(email)) {
            throw new IllegalArgumentException("Email already exists");
        }
        String hash = hashPassword(plainPassword);
        Admin s = new Admin(userId, username, email, hash);
        users.add(s);
        db.saveUsers(users);
        return s;
    }
